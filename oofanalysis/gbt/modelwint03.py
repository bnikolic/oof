...
def MkModel(outputmod=True,
    ...
    if outputmod:
        for el in [ 10 + i*5 for i in range(14)]:
            ...
            iofits4.Write(f, "models/Wint2005V3/surfacepars-el%i.fits" % el ,
                          overwrite=True)
            oofplot.PlotZernFile( "models/Wint2005V3/surfacepars-el%i.fits" % el ,
                                  "models/Wint2005V3/surfacepars-el%i.png/PNG" % el )

            oofplot.PlotZernFile( "models/Wint2005V3/surfacepars-el%i.fits" % el ,
                                  "models/Wint2005V3/surfacepars-el%i.eps/CPS" % el )

            wavel=0.0069569925090132347

            ooffitconv.MkGBTSfcFile( "models/Wint2005V3/surfacepars-el%i.fits" % el ,
                                     "models/Wint2005V3/gbt/surfacepars-el%i.fits" % el ,
                                     wavel)
...
def fitfn( pname,
    ...
    "Fit a hooks type deformation to the data"

    """
    if rigangle is not None than the const coefficient is renormalised
    to make the value of the fit zero at rigangle (units of degrees)
    """
